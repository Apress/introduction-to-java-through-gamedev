...
public final class MyNewGame {
    ...
    public static final void main(String[] args) {
        ...
        try {
            if (MyNewGame.ENGINE_CONFIG_FILE != null && MyNewGame.ENGINE_CONFIG_FILE.equals("") == false) {
                GameSettingsImporter dci = new GameSettingsImporter();
                boolean r = dci.ImportGameSettings(MyNewGame.ENGINE_CONFIG_FILE);
                    ...
                    for (int i = 0; i < len; i++) {
                        try {
                            ...
                            if (ent.from != null && ent.from.equals("GameSettings") == true) {
                                ...
                                f = MyNewGame.class.getField(ent.key);
                                if (f != null) {
                                    System.out.println("Importing " + ent.from + " field: " + ent.key + " with value: " + ent.val + " with type: " + ent.type + " from: " + ent.from);
                                    SetField(ent, f);
                                }             
        ...
        mf = new MainFrame(MyNewGame.WIN_WIDTH, MyNewGame.WIN_HEIGHT, MyNewGame.PANEL_WIDTH, MyNewGame.PANEL_HEIGHT, MyNewGame.GAME_WIDTH, MyNewGame.GAME_HEIGHT);
        pnlGame = new GamePanel(mf, MyNewGame.PANEL_WIDTH, MyNewGame.PANEL_HEIGHT, (MyNewGame.WIN_WIDTH - MyNewGame.PANEL_WIDTH) / 2, (MyNewGame.WIN_HEIGHT - MyNewGame.PANEL_HEIGHT) / 2, MyNewGame.GAME_WIDTH, MyNewGame.GAME_HEIGHT);
        mf.SetGamePanel(pnlGame);
        mf.InitComponents();
        fr = new RunFrameRate(mf, FPS);

        mf.setSize(MyNewGame.WIN_WIDTH, MyNewGame.WIN_HEIGHT);
