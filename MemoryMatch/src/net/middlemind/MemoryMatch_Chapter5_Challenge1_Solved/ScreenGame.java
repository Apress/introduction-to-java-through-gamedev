package net.middlemind.MemoryMatch_Chapter5_Challenge1_Solved;
...
public class ScreenGame extends Screen {
    ...
    * structure used to track clicked cards is an ArrayList. Some of the game programmers wanted to see how the it 
    * would look to use a Stack instead. Refactor the code in the ScreenGame.java class to use a Stack as requested.
    * You'll need to adjust code in a few places throughout the file. The variable used to track the clicked cards
    * is called, clickedCards. 
    * 
    * Keep in mind when using the pop method the value that was stored in the Stack is removed. This means that you'll
    * have to keep track of popped values and restore them if you want to loop over a Stack multiple times. You can keep
    * the MemoryItems popped off of the stack in another another stack or list and add them back in using the Stack 
    * classes addAll method.
    * 
    * You must run this package's file - MemoryMatch.java - right-click and select Run-File, 
    * to test the game. Good luck!
    * 
    * Clue:
    * There are a few ways to find all the places in the file where you have to adjust some code. One way is to change the
    * data type of the variable and look for the errors caused by this change. You could also perform a text search on the
    * name of the variable or right-click the variable and select the "Find Usages" option. You'll have to change the class
    * methods used to get/set values in the data structure from those used by an ArrayList to those used by a Stack.
    * 
    * Search for "CHAPTER 5 CHALLENGE 1 SOLUTION" to find all the solution text.
    */
    ...
    /*********************************************************
    ********* CHAPTER 5 CHALLENGE 1 SOLUTION PART 1 **********
    **********************************************************
    * Solution: 
    * Change the data type of the clickedCards variable to a Stack configured with
    * the same internal data type, the MemoryItem class.
    */     
    
    /**
     * A list of the clicked cards this turn.
     */
    private Stack<MemoryItem> clickedCards;
    ...
    /**
     * A flag used to track if a match was just found.
     */
    ...
    public ScreenGame(GameStates State, GamePanel Owner) {
        ...
        /*********************************************************
        ********* CHAPTER 5 CHALLENGE 1 SOLUTION PART 2 **********
        **********************************************************
        * Solution: 
        * Change the initialization of the clickedCards variable to
        * use the Stack class.
        */
    
        clickedCards = new Stack();
        MemoryItemList.Init();
        pause = false;
        ready = false;
        owner = Owner;
                    ...
                    if(nx >= lx && nx <= lx + lw) {
                        if(ny >= ly && ny <= ly + lh) {
                            ...
                            if(currentPlayerClicks % 2 == 0) {
                                ...
                                /*********************************************************
                                ********* CHAPTER 5 CHALLENGE 1 SOLUTION  PART 3 *********
                                **********************************************************
                                * Solution: 
                                * Change the method used to add a value to the clickedCards
                                * variable. Note that the Stack class also has an add method but
                                * your goal is to use push method.
                                */
    ...
    public void CheckForMatches(MemoryItem itm) {
        if(clickedCards != null) {
            ...
            Stack<MemoryItem> tmp = new Stack();
            matchFound = false;
            
            for(int i = 0; i < len; i++) {
                /*********************************************************
                ********* CHAPTER 5 CHALLENGE 1 SOLUTION PART 4 **********
                **********************************************************
                * Solution: 
                * Change the method used to get values from the data structure
                * to the pop method. Because popping out a value from a stack removes
                * it we need to keep track of the values so we can return them. We'll 
                * use the tmp variable for just that.
                */
                
                MemoryItem tItm = clickedCards.pop();
                ProcessMatchCheck(tItm, itm);
                tmp.push(tItm);
            }
            
            /*********************************************************
            ********* CHAPTER 5 CHALLENGE 1 SOLUTION PART 5 **********
            **********************************************************
            * Solution: 
            * Add the values back into the Stack for later use.
            */
            if(!matchFound) {
                clickedCards.addAll(tmp);
            }
        }
    }
    ...
    public void ProcessMatchCheck(MemoryItem tItm, MemoryItem itm) {
        if(tItm.index == itm.matchIndex || itm.frontStr.equals(tItm.frontStr)) {
            ...
            /*********************************************************
            ********* CHAPTER 5 CHALLENGE 1 SOLUTION  PART 6 *********
            **********************************************************
            * Solution: 
            * Change the method used to add a value to the clickedCards
            * variable. Note that the Stack class also has an add method but
        ...
        if (pause == false && isVisible == true) {
            ...
            if(redXLeft.GetIsVisible() || redXRight.GetIsVisible() || greenCheckLeft.GetIsVisible() || greenCheckRight.GetIsVisible()) {
                if(MmgHelper.CtMs() - showResultStart >= showResultDuration) {
                    ...
                    for(int i = 0; i < len; i++) {
                        /*********************************************************
                        ********* CHAPTER 5 CHALLENGE 1 SOLUTION PART 7 **********
                        **********************************************************
                        * Solution: 
                        * Change the method used to get values from the data structure
                        * to the pop method.
                        */
                        ...
                        clickedCards.pop().flipped = false;
                    }
                    clickFreeze = false;
                }
            }
