...
public final class MemoryMatchTest {

    /*********************************************************
    ********* CHAPTER 7 CHALLENGE 3 SOLUTION PART 1 **********
    **********************************************************
    * Solution:
    * The first step in this solution is to copy the original file and "Refactor Copy"
    * it into the project with a new name, MemoryMatchTest.
    */
    ...
    /*********************************************************
    ********* CHAPTER 7 CHALLENGE 3 SOLUTION PART 2 **********
    **********************************************************
    * Solution:
    * The second part to this solution requires us to change the configuration file
    * and ultimately the image resources associated with this copy of the game to
    * test out a new look.
    */       
    ...
    public static final void main(String[] args) {
        ...
        try {
            if (MemoryMatchTest.ENGINE_CONFIG_FILE != null && MemoryMatchTest.ENGINE_CONFIG_FILE.equals("") == false) {
                GameSettingsImporter dci = new GameSettingsImporter();
                boolean r = dci.ImportGameSettings(MemoryMatchTest.ENGINE_CONFIG_FILE);
                    ...
                    for (int i = 0; i < len; i++) {
                        try {
                            ...
                            if (ent.from != null && ent.from.equals("GameSettings") == true) {
                                ...
                                f = MemoryMatchTest.class.getField(ent.key);
                                if (f != null) {
                                    System.out.println("Importing " + ent.from + " field: " + ent.key + " with value: " + ent.val + " with type: " + ent.type + " from: " + ent.from);
                                    SetField(ent, f);
                                }             
        ...
        mf = new MainFrame(MemoryMatchTest.WIN_WIDTH, MemoryMatchTest.WIN_HEIGHT, MemoryMatchTest.PANEL_WIDTH, MemoryMatchTest.PANEL_HEIGHT, MemoryMatchTest.GAME_WIDTH, MemoryMatchTest.GAME_HEIGHT);
        pnlGame = new GamePanel(mf, MemoryMatchTest.PANEL_WIDTH, MemoryMatchTest.PANEL_HEIGHT, (MemoryMatchTest.WIN_WIDTH - MemoryMatchTest.PANEL_WIDTH) / 2, (MemoryMatchTest.WIN_HEIGHT - MemoryMatchTest.PANEL_HEIGHT) / 2, MemoryMatchTest.GAME_WIDTH, MemoryMatchTest.GAME_HEIGHT);
        mf.SetGamePanel(pnlGame);
        mf.InitComponents();
        fr = new RunFrameRate(mf, FPS);

        mf.setSize(MemoryMatchTest.WIN_WIDTH, MemoryMatchTest.WIN_HEIGHT);
