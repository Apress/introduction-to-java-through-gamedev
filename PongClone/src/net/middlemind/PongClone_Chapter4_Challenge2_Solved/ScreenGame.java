package net.middlemind.PongClone_Chapter4_Challenge2_Solved;
...
public class ScreenGame extends Screen {
    ...
    **********************************************************
    * Description: 
    * Find the package, net.middlemind.PongClone_Chapter4_Challenge2, and open the ScreenGame.java file.
    * One of the lead programmers on the project wants to test making the countdown longer. Your challenge 
    * is to increase the countdown from 3 to 5 seconds. You’ll also update the images used for the count 
    * down so that they don’t have the blue border. This will make our changes stand out a bit.
    * 
    ...
    *       for you change.
    *   3.  Locate the LoadResources method in the ScreenGame class and find 
    *       where the variables number1, number2, number3 are initialized. 
    *       Add number4 and number5 to the variables initialized.
    *   4.  Change the files used to initialize the number 1 – 5 variables by 
    *       adding a 2 before the “png.” For instance, the file name “num_1_lrg.png” 
    *       becomes “num_1_lrg2.png.”
    *   5.  Locate the DrawScreen method and uncomment the new code that draws the 
    *       numbers during the countdown and comment out the old code that only 
    *       supported 3 numbers.
    * 
    * Clue:
    * The best clue to give you for this challenge is to look at the code that is 
    * already in place and use it as a template. In order to run the package’s 
    * specific version of the game you have to click on the static main class 
    * contained in that package and select ‘Run File’ from the context menu.
    * 
    * Search for "CHAPTER 4 CHALLENGE 2 SOLUTION" to find all the solution text.
    */
    
    
    /*********************************************************
    ********** CHAPTER 4 CHALLENGE 2 SOLUTION PART 1 *********
    **********************************************************
    * Solution: 
    * The first part of the solution requires us to add new 
    * enumeration entries for number 4 and 5.
    */    
    ...
    /*********************************************************
    ********** CHAPTER 4 CHALLENGE 2 SOLUTION PART 2 *********
    **********************************************************
    * Solution: 
    * The second part of the solution requires us to add in new class variables,
    * of the MmgBmp - image - data type, to support displaying number 4 and 5.
    */    
    
    /**
     * An MmgBmp image that represents the number 4.
     */    
    private MmgBmp number4;

    /**
     * An MmgBmp image that represents the number 5.
     */
    private MmgBmp number5;       
    ...
    /**
     * A Boolean flag used to test the game that causes the ball to bounce infinitely.
     */    
    ...
    public void LoadResources() {
        ...
        /*********************************************************
        ********** CHAPTER 4 CHALLENGE 2 SOLUTION PART 3 *********
        **********************************************************
        * Solution: 
        * The third part of the solution requires us to initialize the 
        * new number variables in the same we variables number 1 through 
        * 3 are being initialized. Remember you also have to adjust the names
        * of the image resource used to load numbers 1 - 3. Use file names that
        * end in "2.png".
        */        
        
        //Load number four config
        key = "bmpNumberFour";
        if(classConfig.containsKey(key)) {
            file = classConfig.get(key).str;
        } else {
        ...
        number4 = lval;        
        if(number4 != null) {
            MmgHelper.CenterHorAndVert(number4);            
            number4 = MmgHelper.ContainsKeyMmgBmpScaleAndPosition("numberFour", number4, classConfig, number4.GetPosition());
            number4.SetIsVisible(false);
            AddObj(number4);
        }
        
        //Load number five config
        key = "bmpNumberFive";
        if(classConfig.containsKey(key)) {
            file = classConfig.get(key).str;
        } else {
        ...
        number5 = lval;        
        if(number5 != null) {
            MmgHelper.CenterHorAndVert(number5);            
            number5 = MmgHelper.ContainsKeyMmgBmpScaleAndPosition("numberFive", number5, classConfig, number5.GetPosition());
            number5.SetIsVisible(false);
            AddObj(number5);
        }
     ...
     * 
     * @param dir   The D-pad code, UP, DOWN, LEFT, RIGHT of the direction that was pressed on the keyboard.
     * 
     * @return      A Boolean indicating if the D-pad press was handled by this Screen.
     */
    @Override
     ...
     * 
     * @param dir   The D-pad code, UP, DOWN, LEFT, RIGHT of the direction that was released on the keyboard.
     * 
     * @return      A Boolean indicating if the D-pad release was handled by this Screen.
     */    
    @Override
    ...
    public void DrawScreen() {
        ...
        switch(state) {
            ...
            case SHOW_COUNT_DOWN:
                /*********************************************************
                ********** CHAPTER 4 CHALLENGE 2 SOLUTION PART 4 *********
                **********************************************************
                * Solution: 
                * In the last part of this challenge all you had to do was uncomment
                * the new code and comment out the old code.
                */
                ...
                */
                ...
                switch(numberState) {
                    case NONE:
                        // <editor-fold>                        
                        timeNumberMs = System.currentTimeMillis();
                        numberState = NumberState.NUMBER_5;
                        number1.SetIsVisible(false);
                        number2.SetIsVisible(false);
                        number3.SetIsVisible(false);
                        number4.SetIsVisible(false);
                        number5.SetIsVisible(true);
                        if(state == State.SHOW_COUNT_DOWN) {
                            txtGoal.SetIsVisible(true);                            
                            txtDirecP1.SetIsVisible(true);
                            if(gameType == GameType.GAME_TWO_PLAYER) {
                    ...
                    case NUMBER_5:
                        ...
                        if(timeTmpMs - timeNumberMs >= timeNumberDisplayMs) {
                            timeNumberMs = timeTmpMs;
                            numberState = NumberState.NUMBER_4;
                            number1.SetIsVisible(false);
                            number2.SetIsVisible(false);
                            number3.SetIsVisible(false);
                            number4.SetIsVisible(true);
                            number5.SetIsVisible(false);                            
                            if(state == State.SHOW_COUNT_DOWN) {
                                txtGoal.SetIsVisible(true);
                                if(gameType == GameType.GAME_TWO_PLAYER) {
