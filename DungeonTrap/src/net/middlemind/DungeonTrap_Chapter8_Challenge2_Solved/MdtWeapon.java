...
public class MdtWeapon extends MdtBase {
    ...
    public MmgRect GetWeaponRect() {        
        ...
        /*********************************************************
        ********* CHAPTER 8 CHALLENGE 3 SOLUTION PART 2 **********
        **********************************************************
        * Solution:
        * We need to add support to the super-class of all weapons
        * for properly updating and drawing a thrown spear. The spear will
        * rotate in the same way the axe does when thrown. This requires
        * a new rectangle calculation for each rotation of the flame spear's image
        * resources. Recall there is one for each direction.
        */
    
        } else if(attackType == MdtWeaponAttackType.THROWING && (weaponType == MdtWeaponType.AXE || weaponType == MdtWeaponType.SPEAR)) {
            if(throwingFrame == 0) {
    ...
    public void MmgDraw(MmgPen p) {
        if (isVisible == true && active == true) {
            ...
            /*********************************************************
            ********* CHAPTER 8 CHALLENGE 2 SOLUTION PART 3 **********
            **********************************************************
            * Solution:
            * The third step in this solution is to add support for drawing
            * the thrown version of the spear. Notice the adjustment to the
            * if statement is similar to that used in part 2 of the solution.
            */
